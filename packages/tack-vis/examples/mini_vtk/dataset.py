...
class Dataset:
    ...
    Not @tack.data_oriented -- this is a Python-side container.
    The individual components (coordinates, cell_set, arrays) are
    @tack.data_oriented and can be passed into kernels as templates.
    """
...
def make_rectilinear_dataset(x_np, y_np, z_np):
    ...
    coord_field = tack.field(dtype=tack.f32, shape=(len(flat),))
...
def make_explicit_hex_dataset(points_np, connectivity_np):
    ...
    conn_field = tack.field(dtype=tack.i32, shape=(len(connectivity_np),))
