...
@tack.kernel
def _center_scalar(cell_set: tack.template(), cell_type: tack.template(),
...
@tack.kernel
def _center_coords(cell_set: tack.template(), cell_type: tack.template(),
                   coords: tack.template(), center_x, center_y, center_z, n_cells):
    """Interpolate coordinates to cell centers.

    Uses tack.local_array to cache weights -- computed once per cell,
    ...
    for c in range(n_cells):
        ...
        w = tack.local_array(tack.f32, cell_type.num_points)
        pid = tack.local_array(tack.i32, cell_type.num_points)
        for v in range(cell_type.num_points):
...
@tack.kernel
def _center_multi(cell_set: tack.template(), cell_type: tack.template(),
                  field1, field2, field3, out1, out2, out3, n_cells):
    """Interpolate 3 scalar fields to cell centers with cached weights.

    Demonstrates tack.local_array: weights are computed once per cell
...
def parametric_center_multi(dataset, cell_type, names, output_names=None):
    ...
    Uses tack.local_array internally to compute shape function weights
    ...
    o1 = tack.field(dtype=tack.f32, shape=(n,))
    o2 = tack.field(dtype=tack.f32, shape=(n,))
    o3 = tack.field(dtype=tack.f32, shape=(n,))
