...
def compute_normals_gpu(points, conn, n_verts, n_tris):
    ...
    Returns tack.field f32 (n_verts * 3,) with normalized vertex normals.
    """
    normals = tack.field(dtype=tack.f32, shape=(n_verts * 3,))
...
def _apply_transform(points_field, n_verts, transform_4x4):
    ...
    xform = tack.field(dtype=tack.f32, shape=(16,))
...
def _apply_normal_transform(normals_field, n_verts, transform_4x4):
    ...
    nxform = tack.field(dtype=tack.f32, shape=(9,))
...
class Actor:
    ...
    Args:
        points: tack.field f32, shape (n_verts * 3,) — interleaved xyz.
        connectivity: tack.field i32, shape (n_tris * 3,) — triangle indices.
        ...
        point_colors: per-vertex colors. Can be:
            - tack.field f32, shape (n_verts * 3,) — RGB in [0, 1]
        ...
        color_table: :class:`~tack.rendering.ColorTable` instance for
        ...
        normals: pre-computed per-vertex normals. Can be:
            - tack.field f32, shape (n_verts * 3,) — interleaved xyz
    ...
    def __init__(self, points, connectivity, color=(0.8, 0.8, 0.8),
        ...
        if normals is None:
            ...
            self.normals = tack.field(dtype=tack.f32, shape=(n.shape[0],))
        ...
        if point_colors is None:
            ...
            # Convert numpy array (uint8 or float) to tack.field f32
            pc = point_colors.reshape(-1).astype(np.float32)
            if point_colors.dtype == np.uint8:
                pc = pc / 255.0
            self.point_colors = tack.field(dtype=tack.f32, shape=(pc.shape[0],))
            self.point_colors.from_numpy(pc)
        else:
            # Assume tack.field f32
        ...
        if scalars is None:
            ...
            self.scalars = tack.field(dtype=tack.f32, shape=(s.shape[0],))
...
class Scene:
    ...
    def add(self, obj):
        from tack.rendering.volume import Volume
    ...
    def _prepare(self):
        ...
        if len(self.actors) == 1:
            ...
            colors_field = tack.field(dtype=tack.f32, shape=(n_tris * 3,))
            _fill_color(colors_field, 0,
                        actor.color[0], actor.color[1], actor.color[2],
                        n_tris)

            has_normals = 0
            normals_field = tack.field(dtype=tack.f32, shape=(3,))
            ...
            has_point_colors = 0
            pc_field = tack.field(dtype=tack.f32, shape=(3,))
        ...
        points_field = tack.field(dtype=tack.f32, shape=(total_verts * 3,))
        conn_field = tack.field(dtype=tack.i32, shape=(total_tris * 3,))
        colors_field = tack.field(dtype=tack.f32, shape=(total_tris * 3,))
        ...
        if any_has_normals:
            ...
            if all_precomputed:
                ...
                normals_field = tack.field(dtype=tack.f32,
                                          shape=(total_verts * 3,))
                v_off = 0
                for actor in self.actors:
                    nrm = actor.normals
        ...
        pc_field = tack.field(dtype=tack.f32, shape=(3,))
        if any_point_colors:
            pc_field = tack.field(dtype=tack.f32, shape=(total_verts * 3,))
            v_offset = 0
            for actor in self.actors:
                n_v = actor.n_verts
                if actor.point_colors is not None:
        ...
        mat_ids_field = tack.field(dtype=tack.i32, shape=(total_tris,))
        unique_mats = []
        mat_index_map = {}
        tri_offset = 0
        for actor in self.actors:
        ...
        mat_table = tack.field(dtype=tack.f32, shape=(n_mats * 4,))
        mat_table.from_numpy(mat_np)
