...
def _gpu_sort(codes, tri_ids, n):
    ...
    keys = tack.field(dtype=tack.i32, shape=(n_padded,))
    vals = tack.field(dtype=tack.i32, shape=(n_padded,))
...
def _reduce_bounds(centroids, out_min, out_max, n_tris):
    ...
    Since tack.atomic_min/max works on i32, we encode floats as sortable
    ...
    for i in range(n_tris):
        ...
        cy = centroids[i * 3 + 1]
        cz = centroids[i * 3 + 2]
        tack.atomic_min(out_min, 0, cx)
        tack.atomic_min(out_min, 1, cy)
        tack.atomic_min(out_min, 2, cz)
        tack.atomic_max(out_max, 0, cx)
        tack.atomic_max(out_max, 1, cy)
        tack.atomic_max(out_max, 2, cz)
...
class BVH:
    ...
    After ``build()``, the following tack fields are available:
    ...
    def build(self, points, conn, n_tris, gpu_sort=True):
        """Build BVH from tack fields.

        Args:
            points: tack.field f32, shape (n_verts * 3,) — interleaved xyz.
            conn: tack.field i32, shape (n_tris * 3,) — triangle indices.
        ...
        centroids = tack.field(dtype=tack.f32, shape=(n_tris * 3,))
        tri_aabbs = tack.field(dtype=tack.f32, shape=(n_tris * 6,))
        ...
        out_min = tack.field(dtype=tack.f32, shape=(3,))
        out_max = tack.field(dtype=tack.f32, shape=(3,))
        ...
        codes = tack.field(dtype=tack.i32, shape=(n_tris,))
        ...
        tri_ids = tack.field(dtype=tack.i32, shape=(n_tris,))
        ...
        if gpu_sort:
            ...
            sorted_codes = tack.field(dtype=tack.i32, shape=(n_tris,))
