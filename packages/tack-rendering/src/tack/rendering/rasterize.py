...
def _build_mvp(camera):
    ...
    from tack.rendering.camera import OrthographicCamera
    ...
    for actor in scene.actors:
        if actor.render_mode == "wireframe":
            colors_field = canvas.get_work_buffer(
                f'wire_col_{id(actor)}', tack.f32, (actor.n_tris * 3,))
            from tack.rendering.scene import _fill_color
