...
and Canvas from tack.rendering.
"""
...
class Volume:
    ...
    Args:
        scalar_field: tack.field f32 or numpy array holding the 3D scalar
    ...
    def __init__(self, scalar_field, dims, origin=(0, 0, 0),
        ...
        if isinstance(scalar_field, np.ndarray):
            ...
            f = tack.field(dtype=tack.f32, shape=(flat.shape[0],))
            f.from_numpy(flat)
            self.scalar_field = f
        else:
        ...
        self._texture = tack.texture3d(self.scalar_field,
...
def render_volume(canvas, volume, camera, background=(0.05, 0.05, 0.1)):
    ...
    fb_r = canvas.get_work_buffer('vol_fb_r', tack.f32, (n_pixels,))
