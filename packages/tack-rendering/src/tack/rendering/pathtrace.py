...
def _pathtrace(fb_r, fb_g, fb_b, fb_depth,
               ...
               mat_ids, mat_table,
               stack,
               camera: tack.template(),
               config: tack.template(),
...
def render(canvas, scene, camera, samples=1, max_bounces=3,
    ...
    from tack.rendering.scene import DirectionalLight
    ...
    light_data = canvas.get_work_buffer('light_data', tack.f32,
                                        (light_np.shape[0],))
    light_data.from_numpy(light_np)

    # Volume data (first volume in scene, or dummy)
    has_volume = 0
    vol_tf = canvas.get_work_buffer('vol_tf_dummy', tack.f32, (4,))
    vol_data = canvas.get_work_buffer('vol_data_dummy', tack.f32, (1,))
    if scene.volumes:
        vol = scene.volumes[0]
    ...
    fb_r = canvas.get_work_buffer('fb_r', tack.f32, (n_pixels,))
    fb_g = canvas.get_work_buffer('fb_g', tack.f32, (n_pixels,))
    fb_b = canvas.get_work_buffer('fb_b', tack.f32, (n_pixels,))

    # Depth buffer (cached)
    fb_depth = canvas.get_work_buffer('fb_depth', tack.f32, (n_pixels,))

    # BVH traversal stack (cached)
    stack = canvas.get_work_buffer('stack', tack.i32,
