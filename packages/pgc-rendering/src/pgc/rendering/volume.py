...
def _volume_render(fb_r, fb_g, fb_b,
    ...
    for pid in range(n_pixels):
        ...
        # Ray from camera (same convention as path tracer)
        rdx = camera.corner_x + camera.dx_x * (float(px) + 0.5) + camera.dy_x * (float(py) + 0.5)
        rdy = camera.corner_y + camera.dx_y * (float(px) + 0.5) + camera.dy_y * (float(py) + 0.5)
        rdz = camera.corner_z + camera.dx_z * (float(px) + 0.5) + camera.dy_z * (float(py) + 0.5)
