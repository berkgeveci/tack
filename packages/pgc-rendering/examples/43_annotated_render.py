...
sp = pgc.field(dtype=pgc.f32, shape=(sv.size,))
sp.from_numpy(sv.reshape(-1))
sc = pgc.field(dtype=pgc.i32, shape=(st.size,))
...
pp = pgc.field(dtype=pgc.f32, shape=(pv.size,))
pp.from_numpy(pv.reshape(-1))
pc = pgc.field(dtype=pgc.i32, shape=(pt.size,))
...
image = annotate(image, [
    ...
    TextOverlay("PGC Renderer", position=(10, 10), font_size=18),
])
