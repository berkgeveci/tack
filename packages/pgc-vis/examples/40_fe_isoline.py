"""Extract isolines from a high-order DG field using pgc.fe.
...
import os
import pgc
from pgc.fe.basis import QuadBasis, precompute_basis_matrix_2d
        ...
        for idx in range(elem_indices.shape[0]):
            ...
            for sj in range(n_sub):
                for si in range(n_sub):
                    ...
                    cr = pgc.shared_like(out_x0, 4)
                    cs = pgc.shared_like(out_x0, 4)
                    nc = 0
...
def main():
    ...
    f_x0 = pgc.field(dtype=pgc_fp, shape=(total_segs,))
    f_y0 = pgc.field(dtype=pgc_fp, shape=(total_segs,))
    f_x1 = pgc.field(dtype=pgc_fp, shape=(total_segs,))
    f_y1 = pgc.field(dtype=pgc_fp, shape=(total_segs,))
