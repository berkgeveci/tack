"""Extract isosurfaces from a high-order DG field on hexes using pgc.fe.
...
import pgc
from pgc.fe.basis import HexBasis, precompute_basis_matrix_3d
from pgc.fe.accessor import contiguous_from_numpy
from pgc.fe.geometry import linear_hex_map_from_numpy
...
def main():
    ...
    parser.add_argument("--arch", default=os.environ.get("PGC_ARCH", "cpu"))
    ...
    pgc_fp = pgc.f32 if args.arch == "metal" else pgc.f64
    ...
    ndof = basis.n1d ** 3
    n_pts = (args.nsub + 1) ** 3
    f_B = pgc.field_like(B.ravel().astype(np_fp))
    f_elem_idx = pgc.field_like(elem_idx)

    # MC tables
    f_num_tris = pgc.field_like(NUM_TRIS)
    f_tri_table = pgc.field_like(TRI_TABLE)
    f_edge_verts = pgc.field_like(EDGE_VERTS)

    eval_field, count_tris_k, gen_tris = define_kernels()

    # Evaluate field
    f_field_vals = pgc.field(dtype=pgc_fp, shape=(n_elems * n_pts,))
    eval_field(accessor, f_B, ndof, n_pts, f_elem_idx, f_field_vals)

    # Count
    f_counts = pgc.field(dtype=pgc.i32, shape=(n_elems,))
    ...
    f_offsets = pgc.field_like(offsets)

    n_out = total_tris * 3
    f_px = pgc.field(dtype=pgc_fp, shape=(n_out,))
    f_py = pgc.field(dtype=pgc_fp, shape=(n_out,))
    f_pz = pgc.field(dtype=pgc_fp, shape=(n_out,))
