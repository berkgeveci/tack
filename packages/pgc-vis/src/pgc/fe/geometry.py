...
class LinearQuadMap:
    ...
    def __init__(self, vx, vy, connectivity, conn_offsets):
        ...
        Args:
            vx, vy: pgc.field of vertex coordinates.
            connectivity: pgc.field(i32) of element-to-vertex indices.
            conn_offsets: pgc.field(i32) of per-element offsets.
        """
        self.vx = vx
        self.vy = vy
...
def linear_quad_map_from_numpy(vx_np, vy_np, conn_np, conn_off_np,
    ...
    return LinearQuadMap(
        vx=pgc.field_like(vx_np.astype(np_fp)),
        vy=pgc.field_like(vy_np.astype(np_fp)),
        connectivity=pgc.field_like(conn_np.astype(np.int32)),
        conn_offsets=pgc.field_like(conn_off_np.astype(np.int32)))
...
def linear_hex_map_from_numpy(vx_np, vy_np, vz_np, conn_np, conn_off_np,
    ...
    return LinearHexMap(
        vx=pgc.field_like(vx_np.astype(np_fp)),
        vy=pgc.field_like(vy_np.astype(np_fp)),
        vz=pgc.field_like(vz_np.astype(np_fp)),
        connectivity=pgc.field_like(conn_np.astype(np.int32)),
        conn_offsets=pgc.field_like(conn_off_np.astype(np.int32)))
