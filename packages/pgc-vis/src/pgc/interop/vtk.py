"""pgc.interop.vtk — Zero-copy interop between VTK arrays and PGC fields.
...
::
    ...
    # VTK → PGC (zero-copy for host and device arrays)
    field = vtk_to_field(vtk_data_array)

    # PGC → VTK (zero-copy for host arrays)
...
# VTK type enum → (numpy dtype, pgc dtype)
_VTK_TYPE_MAP = {
    10: (np.float32, pgc.f32),   # VTK_FLOAT
    11: (np.float64, pgc.f64),   # VTK_DOUBLE
    6:  (np.int32,   pgc.i32),   # VTK_INT
    8:  (np.int64,   pgc.i64),   # VTK_LONG_LONG (or VTK_ID_TYPE on 64-bit)
    12: (np.int64,   pgc.i64),   # VTK_ID_TYPE
}
...
def vtk_to_field(vtk_array):
    """Convert a vtkDataArray to a pgc.field (zero-copy when possible).
    ...
    Returns:
        pgc.field wrapping the array's memory (zero-copy).
...
def field_to_vtk(field, n_components=1):
    """Create a vtkDataArray wrapping a pgc.field's memory (zero-copy).
