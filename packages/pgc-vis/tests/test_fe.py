"""Tests for pgc.fe — finite element basis, accessor, geometry."""

import numpy as np
import pgc
from pgc.fe.basis import (
    QuadBasis, HexBasis,
...
def test_lagrange_1d_cardinal():
    ...
    @pgc.kernel
    def check_cardinal(b: pgc.template(), out):
        # Evaluate basis function i at node j → should be delta_ij
        n = b.n1d
        for i in range(n):
            for j in range(n):
                r = b.gl_nodes_1d[j]
                val = pgc.fe.basis.lagrange_1d(b.gl_nodes_1d, i, r, n)
                out[i * n + j] = val

    # Can't call pgc.func from kernel with module path.
    # Instead, test via the QuadBasis.evaluate at corner nodes.
    @pgc.kernel
    def check_quad_cardinal(b: pgc.template(), out, ndof):
    ...
    n1d = 3
    ndof = n1d * n1d
    out = pgc.field(dtype=pgc.f32 if np_fp == np.float32 else pgc.f64,
                    shape=(ndof * ndof,))
...
def test_quad_partition_of_unity():
    ...
    @pgc.kernel
    def check_partition(b: pgc.template(), out, ndof):
        for idx in range(25):
            r = float(idx % 5) * 0.25
            s = float(idx // 5) * 0.25
            total = 0.0
...
def test_contiguous_accessor():
    ...
    @pgc.kernel
    def read_dofs(acc: pgc.template(), out):
        for i in range(1):
            out[0] = acc.get_dof(1, 2)
            out[1] = acc.get_dof(2, 0)

    fp = pgc.f32 if np_fp == np.float32 else pgc.f64
    out = pgc.field(dtype=fp, shape=(2,))
...
def test_linear_quad_map():
    ...
    @pgc.kernel
    def eval_map(g: pgc.template(), out):
        for i in range(1):
    ...
    fp = pgc.f32 if np_fp == np.float32 else pgc.f64
    out = pgc.field(dtype=fp, shape=(6,))
    eval_map(gmap, out)
    result = out.to_numpy()
