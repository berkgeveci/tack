"""GPU-accelerated statistics and analysis on pgc fields.
...
def _sum_sq_diff(data, mean_val, out, n):
    ...
    for i in range(n):
        ...
        pgc.atomic_add(out, 0, diff * diff)
...
def _sq_sum(data, out, n):
    """Sum of squares: Σx[i]²."""
    for i in range(n):
        pgc.atomic_add(out, 0, data[i] * data[i])
...
def var(data, n=None):
    ...
    if n is None:
        n = data.size
    mean_val = data.sum() / n
    acc = pgc.field(dtype=pgc.f32, shape=(1,))
    acc.fill(0.0)
...
def count_nonzero(data, n=None):
    ...
    if n is None:
        n = data.size
    acc = pgc.field(dtype=pgc.i32, shape=(1,))
    acc.fill(0)
...
def histogram(data, bins=10, range=None, n=None):
    ...
    Returns:
        (counts, bin_edges) where counts is a pgc.field of i32,
    ...
    counts = pgc.field(dtype=pgc.i32, shape=(bins,))
    counts.fill(0)
