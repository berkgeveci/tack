"""Tests for GPU statistics and analysis (pgc.algorithms.stats)."""
...
def test_norm_l2(backend):
    f = pgc.field(dtype=pgc.f32, shape=(3,))
...
def test_count_nonzero(backend):
    f = pgc.field(dtype=pgc.f32, shape=(6,))
...
def test_dot_basic(backend):
    ...
    b = pgc.field(dtype=pgc.f32, shape=(3,))
