...
Demonstrates using @tack.data_oriented templates to build a generic
...
@tack.data_oriented
class AOSArray:
    """Backed by a tack.field -- standard memory-resident array."""
...
@tack.kernel
def add_arrays(input1: tack.template(), input2: tack.template(),
               output: tack.template(), n):
...
@tack.kernel
def scale_array(input1: tack.template(), scalar: tack.template(),
                output: tack.template(), n):
...
a_data = tack.field(dtype=tack.f32, shape=(n,))
b_data = tack.field(dtype=tack.f32, shape=(n,))
out_data = tack.field(dtype=tack.f32, shape=(n,))
