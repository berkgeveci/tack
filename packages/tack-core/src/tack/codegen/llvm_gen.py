"""Tack LLVM IR code generation — transforms Tack IR to LLVM IR via llvmlite.
...
def _llvm_type(tack_type: ScalarType) -> llvm_ir.Type:
    """Convert a Tack scalar type to an LLVM IR type."""
    if tack_type is f32:
        return llvm_ir.FloatType()
    if tack_type is f64:
        return llvm_ir.DoubleType()
    if tack_type in (i8, u8):
        return llvm_ir.IntType(8)
    if tack_type in (i16, u16):
        return llvm_ir.IntType(16)
    if tack_type in (i32, u32):
        return llvm_ir.IntType(32)
    if tack_type in (i64, u64):
        return llvm_ir.IntType(64)
    raise TypeError(f"No LLVM type mapping for {tack_type}")
...
class LLVMCodeGen:
    """Generates LLVM IR from Tack IR for a single kernel function.
    ...
    def __init__(self, ir_func: ir.IRFunction):
        ...
        # Maps Tack parameter names to their types (for knowing element type of fields)
    ...
    def generate(self) -> llvm_ir.Module:
        ...
        for param in func.params:
            tack_type = param.type_annotation
            if tack_type is None:
                raise TypeError(f"Parameter '{param.name}' has no type annotation. "
                                "Run type inference first.")
            self._param_types[param.name] = tack_type
            elem_llvm = _llvm_type(tack_type)
