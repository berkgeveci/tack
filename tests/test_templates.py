...
class CellSetExplicit:
    def __init__(self, conn_np):
        self.connectivity = pgc.field(dtype=pgc.i32, shape=(len(conn_np),))
...
def test_structured_template():
    ...
    nx, ny = 4, 3
    num_cells = (nx - 1) * (ny - 1)
    output = pgc.field(dtype=pgc.i32, shape=(num_cells,))
...
def test_same_kernel_different_templates():
    ...
    output_s = pgc.field(dtype=pgc.i32, shape=(num_cells,))
    output_e = pgc.field(dtype=pgc.i32, shape=(num_cells,))
...
def test_different_scalar_values():
    ...
    output_a = pgc.field(dtype=pgc.i32, shape=(6,))
    output_b = pgc.field(dtype=pgc.i32, shape=(12,))
