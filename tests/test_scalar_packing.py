...
def test_many_float_scalars():
    """Kernel with 20 float scalar params — all packed into __pack_f32__."""
    n = 100
...
def test_mixed_int_and_float_scalars():
...
def test_scalar_packing_called_twice():
    """Verify packed kernels work correctly on repeated calls with different values."""
    n = 32
    out = pgc.field(dtype=pgc.f32, shape=(n,))
    ...
    np.testing.assert_allclose(r1, 3.0 * np.arange(n, dtype=np.float32) + 1.0, rtol=1e-5)
    np.testing.assert_allclose(r2, -1.0 * np.arange(n, dtype=np.float32) + 100.0, rtol=1e-5)


def test_no_scalars_no_packing():
    """Kernel with only field params — packing should be a no-op."""
    n = 64
...
def test_numpy_scalar_types():
    """numpy scalar types (np.float32, np.int32) should work as kernel args."""
    n = 16
    out = pgc.field(dtype=pgc.f32, shape=(n,))
