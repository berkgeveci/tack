...
compute_normals(points, conn, n_pts, n_tris)
    Returns pgc.field of interleaved normals (n_pts*3,) f32.
"""
...
def compute_normals(points, conn, n_pts, n_tris):
    ...
    Args:
        points: pgc.field, interleaved (n_pts*3,) f32
        conn: pgc.field, interleaved (n_tris*3,) i32
        n_pts: number of vertices
        n_tris: number of triangles

    Returns:
        pgc.field, interleaved normals (n_pts*3,) f32
    """
    normals = pgc.field(dtype=pgc.f32, shape=(n_pts * 3,))
    normals.fill(0)
