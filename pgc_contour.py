"""
PGC-based 2D contour (marching squares) on quad meshes.

Port of the Taichi contour example demonstrating the Viskores design pattern:
...
1. CellSetStructured2D — connectivity computed from dimensions
...
class MarchingSquaresTables:
    ...
    def __init__(self):
        self.num_lines = pgc.field(dtype=pgc.i32, shape=(16,))
        # Flatten (16, 2, 2) → (64,) for 1D field indexing
        self.segments = pgc.field(dtype=pgc.i32, shape=(64,))
        # Flatten (4, 2) → (8,)
        self.edge_verts = pgc.field(dtype=pgc.i32, shape=(8,))
...
# resolved at JIT time per type via @pgc.data_oriented templates.
...
class CellSetStructured2D:
    ...
    def get_cell_point(self, cell_id, local_idx):
        ...
        # Use conditional chain instead of vector return
        result = p0
        if local_idx == 1:
            result = p0 + 1
        if local_idx == 2:
...
class CellSetExplicitQuads:
    ...
    def __init__(self, connectivity_np, num_cells, num_points):
        self.connectivity = pgc.field(dtype=pgc.i32, shape=(len(connectivity_np),))
...
def run_contour(cell_set, coords_x, coords_y, scalar_field, isovalue,
    ...
    # Store isovalue in a 1-element field (scalar args not yet supported)
    ...
    # Scatter: host-side prefix sum
    nl = num_lines_f.to_numpy()
    offsets = np.zeros(nc, dtype=np.int32)
    if nc > 1:
        offsets[1:] = np.cumsum(nl[:-1])
    total_lines = int(offsets[-1] + nl[-1]) if nc > 0 else 0
    total_points = total_lines * 2
    scatter_offsets_f.from_numpy(offsets)
...
if __name__ == "__main__":
    ...
    # --- allocate fields ---
    coords_x = pgc.field(dtype=pgc.f32, shape=(NUM_POINTS,))
    coords_y = pgc.field(dtype=pgc.f32, shape=(NUM_POINTS,))
    scalar_field = pgc.field(dtype=pgc.f32, shape=(NUM_POINTS,))

    num_lines_f = pgc.field(dtype=pgc.i32, shape=(NUM_CELLS,))
    scatter_offsets_f = pgc.field(dtype=pgc.i32, shape=(NUM_CELLS,))
    interp_pa = pgc.field(dtype=pgc.i32, shape=(MAX_POINTS,))
    interp_pb = pgc.field(dtype=pgc.i32, shape=(MAX_POINTS,))
    interp_w = pgc.field(dtype=pgc.f32, shape=(MAX_POINTS,))
    output_x = pgc.field(dtype=pgc.f32, shape=(MAX_POINTS,))
    output_y = pgc.field(dtype=pgc.f32, shape=(MAX_POINTS,))
