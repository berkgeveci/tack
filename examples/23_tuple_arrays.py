...
def make_soa_type(nc):
    """Generate a @pgc.data_oriented SOA class with nc component fields."""
    if nc in _soa_cache:
...
@pgc.kernel
def compute_magnitude(arr: pgc.template(), output, n):
...
@pgc.kernel
def add_tuple_arrays(a: pgc.template(), b: pgc.template(),
                     out: pgc.template(), n, nc):
...
@pgc.kernel
def scale_tuple_array(arr: pgc.template(), factor, out: pgc.template(), n, nc):
...
@pgc.kernel
def dot_product(a: pgc.template(), b: pgc.template(), output, n, nc):
...
def make_aos(data_np):
    ...
    nt, nc = data_np.shape
    field = pgc.field(dtype=pgc.f32, shape=(nt * nc,))
...
def make_soa(data_np):
    ...
    fields = []
    for c in range(nc):
        f = pgc.field(dtype=pgc.f32, shape=(nt,))
...
output = pgc.field(dtype=pgc.f32, shape=(n,))
...
a = make_aos(data_a)
b = make_aos(data_b)
out_field = pgc.field(dtype=pgc.f32, shape=(n * nc,))
...
b = make_soa(data_b)
out = SOAType(
    pgc.field(dtype=pgc.f32, shape=(n,)),
    pgc.field(dtype=pgc.f32, shape=(n,)),
    pgc.field(dtype=pgc.f32, shape=(n,)),
)
...
def make_soa_output(nc, n):
    return SOAType(*[pgc.field(dtype=pgc.f32, shape=(n,)) for _ in range(nc)])

aos_a = make_aos(data_a)
soa_b = make_soa(data_b)
aos_out_field = pgc.field(dtype=pgc.f32, shape=(n * nc,))
...
soa5_out = SOA5(*[pgc.field(dtype=pgc.f32, shape=(n,)) for _ in range(nc5)])
...
make_soa_type(nc) generates @pgc.data_oriented classes for any number
