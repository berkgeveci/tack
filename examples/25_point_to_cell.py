...
@pgc.kernel
def point_to_cell_average(cell_set: pgc.template(), point_data, cell_data, n_cells):
...
@pgc.kernel
def build_explicit_connectivity(struct: pgc.template(), conn, n_cells):
...
cell_data = pgc.field(dtype=pgc.f32, shape=(n_cells,))
...
print("\nExplicit cell set...")
conn_field = pgc.field(dtype=pgc.i32, shape=(n_cells * 8,))
...
(explicit connectivity array).  Multi-return @pgc.func allows
...
@pgc.kernel
def point_to_cell_average_mc(cell_set: pgc.template(),
                             point_data: pgc.template(),
                             cell_data: pgc.template(),
                             n_cells, nc):
...
aos_pt_field = pgc.field(dtype=pgc.f32, shape=(n_points * nc,))
aos_pt_field.from_numpy(pd_mc.ravel())
aos_pt = AOSTupleArray(aos_pt_field, n_points, nc)

aos_cell_field = pgc.field(dtype=pgc.f32, shape=(n_cells * nc,))
...
soa_pt_fields = [pgc.field(dtype=pgc.f32, shape=(n_points,)) for _ in range(nc)]
for c in range(nc):
    soa_pt_fields[c].from_numpy(pd_mc[:, c].copy())
soa_pt = SOATupleArray3(*soa_pt_fields)

soa_cell_fields = [pgc.field(dtype=pgc.f32, shape=(n_cells,)) for _ in range(nc)]
...
aos_cell_field2 = pgc.field(dtype=pgc.f32, shape=(n_cells * nc,))
